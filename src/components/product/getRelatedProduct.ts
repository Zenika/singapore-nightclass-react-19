...
export function getRelated_PROMISE() {
	return timeout(1000)
		.then(() =>
			fetch(
				"https://mock.shop/api?https://mock.shop/api?query=%7B%20products(first%3A%203%2C%20query%3A%20%22NOT%20gid%3A%2F%2Fshopify%2FProduct%2F7982905098262%22)%20%7B%20edges%20%7B%20node%20%7B%20id%20title%20featuredImage%20%7B%20id%20url%20%7D%20description%20%7D%20%7D%20%7D%7D",
			),
		)
		.then((res) => {
			if (!res.ok) {
